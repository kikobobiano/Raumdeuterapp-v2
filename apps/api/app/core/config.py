...
# Squad Value page/API — excluded entirely (not just xTV masking).
LEAGUES_EXCLUDED_FROM_SQUAD_VALUE: frozenset[str] = LEAGUES_WITHOUT_XTV
...
# Side-by-side Compare presets (legacy PRESETS_RAW / frontend POSITION_PRESETS).
# Profile.table is built from these; override via ?table_role= for cross-position compare.
