    ...
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-2 pb-1 text-[11px] text-content-muted">
        ...
        <span className="ml-auto text-[10px] uppercase tracking-widest">
          ...
          {domesticOnly && domesticCountry
            ? ` · ${domesticCountry} passport only`
            : domesticOnly
              ? " · domestic only"
              : ""}
        </span>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                "grid-cols-[28px_minmax(0,160px)_minmax(0,1fr)]",
      ...
      <p className="mt-2 px-2 text-[10px] text-content-muted">
        {domesticOnly
          ? "Age-band shares count only minutes from domestic-passport players. "
          : ""}
        Each bar shows the % of that club&apos;s squad minutes given to each
        age band (labelled inside the segment when wide enough). Click a row to
        inspect the full squad.
      </p>
    </div>
  );
}
